import { QUIZ_PUZZLE_VALUE_REGEX, QUIZ_PUZZLE_VALUES_MAX } from '@quiz/common'
import React, {
  FC,
  useCallback,
  useEffect,
...
const PuzzleValues: FC<PuzzleValuesProps> = ({
  ...
  const isRequired = useCallback(
    (index: number): boolean => (initialValues?.length || 3) > index,
    [initialValues],
  )
    ...
    (updatedIndex: number, newValue?: string) => {
      ...
      const indexToRemove =
        reversedIndexToRemove >= 0
          ? Math.max(values.length - reversedIndexToRemove, 1)
          : 1
