...
docker build --platform linux/arm64 -t $QUIZ_SERVICE_REPOSITORY:$TAG -f ./packages/$QUIZ_SERVICE_PACKAGE/Dockerfile . || exit 1
...
git add "$QUIZ_SERVICE_DEPLOYMENT_FILE" && git commit --no-verify -m "Promote $QUIZ_SERVICE_PACKAGE to k3s"

sed -i "s|image:.*|image: $REGISTRY/$QUIZ_REPOSITORY:$TAG|" "$QUIZ_DEPLOYMENT_FILE"
git add "$QUIZ_DEPLOYMENT_FILE" && git commit --no-verify -m "Promote $QUIZ_PACKAGE to k3s"
